...
comsol_data = Path(current_folder.parents[1], 'comsol_data', 'spr_1d_comsol', 'SPR_homogeneous.txt')
...
with open(str(comsol_data), 'r') as file:
    for i, line in enumerate(file):
        if i > 4:
            ...
            n_channel.append(float(line_array[1]))
            R.append(float(line_array[3]))

unique_in_rad = np.unique(in_rad)
unique_n_channel  = np.unique(n_channel)
R_matrix = np.zeros(shape=(len(unique_n_channel), len(unique_in_rad)))

for j in range(len(unique_in_rad)):
    for i in range(len(unique_n_channel)):
        R_matrix[i, j] = R[j*len(unique_n_channel) + i]


unique_in_deg = unique_in_rad*180/PI
unique_in_deg = unique_in_deg
R_matrix = R_matrix.T
extent = np.array([unique_n_channel.min(), unique_n_channel.max(), unique_in_deg.min(), unique_in_deg.max()])
plt.imshow(R_matrix, origin='lower', aspect='auto', cmap=cmap, interpolation='spline16', extent=extent)

plt.xlabel(r'Refractive index [-]')
plt.ylabel(r'$\theta_{in}$ [$^\circ$]')

# plt.xlim([1.10*180/PI, 1.11*180/PI])
...
minimums = np.zeros(len(unique_in_deg))
# for i in np.array([0, 2, 4, 6, 8]):
#     minimums[i] = unique_sensor_response[np.argmin(R_matrix[:, i])]
#     plt.plot(unique_sensor_response, R_matrix[:, i], label=r'$h_{BSA}$ ' + str(round(unique_h_bsa[i] - 1)) + ' nm')


# for i in np.array([0, 2]):
#     minimums[i] = unique_sensor_response[np.argmin(R_matrix[:, i])]
#     plt.plot(unique_sensor_response -61, R_matrix[:, i], label=r'$h_{BSA}$ ' + str(round(unique_h_bsa[i] - 1)) + ' nm')
...
plt.xlabel(r'Sensor response [$\mu$m]')
plt.legend()
plt.grid(linewidth=1, alpha=0.3)
plt.tight_layout()

# plt.savefig('resonance_shift_BSA.png', format='png', dpi=600)
