...
def isolate_SPR(peak_coords, peak_values, width_around_SPR_dip_um):
    ...
    ## Provided SPR is the minimum...
    spr_dip_index = np.argmin(peak_values[start_from:-stopp_at])
    spr_dip_index = spr_dip_index + start_from
    
    ## x coordinates for isolated spectrum around SPR dip
    # spr_x = np.arange(peak_coords[spr_dip_index] - width_around_SPR_dip/2,
                      # peak_coords[spr_dip_index] + width_around_SPR_dip/2, 
                      # 15)
                      
            
    spr_x = peak_coords[spr_dip_index - width_around_SPR_dip_px//2:spr_dip_index + width_around_SPR_dip_px//2]
    
    ## If minimum can not be found return all zeros
    if np.any(spr_dip_index < 0): 
        return 0, 0, 0
    
    if spr_dip_index < 2:
        spr_y = np.zeros(shape=len(spr_x))
        print('SPR dip is in the corner of the peak spectrum')
    else:   
        ## Values around the SPR dip
        spr_y = peak_values[spr_dip_index - len(spr_x)//2:spr_dip_index + len(spr_x)//2]
...
class SPR_figure():
    ...
    def analyze_image(self, 
        ...
        ## Crop image around laser beam of choice
        cropped_image = image[int(y_max_index-self.integrate_over_pixel):int(y_max_index+self.integrate_over_pixel), :]

        ## Integrate and create x-coordinates for cropped image
        ...
        ## Find gold lines
        peak_x, peak_y = create_peak_spectrum(x, y, 
        ...
        ## Find location of SPR!
        spr_location = find_SPR_dip(spr_x, np.max(spr_y) - spr_y)
    
        if frame_counter == 0:
            ## First cropped image
            self.im_raw_data = self.ax_array[0].imshow(image, extent=extent_raw, origin='lower') 
            ...
            ## Isolated SPR dip
            self.dip_spectrum, = self.ax_array[3].plot(spr_x, spr_y, 'r', marker='o',markersize=3)
            
            ## Plot lines in vertical integrated spectrum
            self.ax_array[1].plot(np.array([start_look_for_dip_from, start_look_for_dip_from]),
                                  np.array([np.min(y), np.max(y)]), 
                                  '--', color='black', linewidth=1)
            self.ax_array[1].plot(np.array([stopp_look_for_dip_from, stopp_look_for_dip_from]), 
                                  np.array([np.min(y), np.max(y)]),
                                  '--', color='black', linewidth=1)
            
            self.left_line_peak_spectrum, = self.ax_array[2].plot(np.array([np.min(spr_x), np.min(spr_x)]),
                                                                  np.array([np.min(peak_y), np.max(peak_y)]), 
                                                                  '--', color='black', linewidth=1)
            self.right_line_peak_spectrum, = self.ax_array[2].plot(np.array([np.max(spr_x), np.max(spr_x)]),
                                                                   np.array([np.min(peak_y), np.max(peak_y)]), 
                                                                   '--', color='black', linewidth=1)
            
        else:
            ## For some reason this takes a lot of time!
            # self.im_raw_data = self.ax_array[0].imshow(image, extent=extent_raw, origin='lower')
            ...
            self.dip_spectrum.set_data(spr_x, spr_y)
            self.ax_array[3].set_xlim([np.min(spr_x), np.max(spr_x)])
            self.ax_array[3].set_ylim([np.min(spr_y), np.max(spr_y)])
            
            self.left_line_peak_spectrum.set_data(np.array([np.min(spr_x), np.min(spr_x)]), np.array([np.min(peak_y), np.max(peak_y)]))
            self.right_line_peak_spectrum.set_data(np.array([np.max(spr_x), np.max(spr_x)]), np.array([np.min(peak_y), np.max(peak_y)]))
            ...
            if run_measurement:
                ...
                while (start_time - measurement_time_start) < measurement_time:
                    ...
                    for laser in lasers_on_chip:
                        ...
                        # for i in range(len(lasers_on_chip)):
                        results['frame_time'][laser].append(frame_time)
                        results['spr_data'][laser].append(spr_figure.analyze_image(image, 
