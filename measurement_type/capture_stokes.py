...
def measurement_main(config: dict, meas_output_dir_path: str):
    ...
    while True:
        ...
        if key == 'c':
            ...
            full_image_I.append(np.sum(np.sum(image)))
            ...
            for roi in ROI:
                ...
                roi_image_I.append(np.sum(np.sum(roi_image)))
                roi_counter += 1
        ...
        if key == 'q':
            vp.headline('Shutting down and plotting intensities')
            plt.figure(0)
            full_image_I = np.array(full_image_I)
            roi_image_I  = np.array(roi_image_I)
            plt.plot(full_image_I, 'black')
            plt.plot(roi_image_I, 'red')
            break
