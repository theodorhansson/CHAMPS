...
int sample_capture_and_access_image(HDCAM hdcam, HDCAMWAIT hwait, int32 nFrame)
{
	...
	if (failed(err))
	{
		dcamcon_show_dcamerr(hdcam, err, "dcamcap_start(SNAP)");
		ret = 1;
	}
	else
	{
		...
		while (1)
		{
			if (nAccessFrameCount < nFrame)
			{
				...
				{
					DCAMCAP_TRANSFERINFO	captrans;
					memset(&captrans, 0, sizeof(captrans));
					captrans.size = sizeof(captrans);

					err = dcamcap_transferinfo(hdcam, &captrans);
					if (failed(err))
					{
					...
					{
						while (nAccessFrameCount < captrans.nFrameCount)
						{
							// copy image
